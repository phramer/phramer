...
def preprocess_corpus(data_path, new_path, workers=50):
    ...
    preprocessed_folder_path = (
        data_path[: data_path.rfind("/") + 1] + "preprocessed/"
    )
    ...
    try:
        ...
        print(
            "Successfully created the directory %s " % preprocessed_folder_path
        )
    ...
    processed_files = sorted(
        processed_files, key=lambda file: int(file[file.rfind("_") + 1 :])
    )
    f = open(new_path, "w")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to data to preprocess",
    )
    parser.add_argument(
        "--new_path",
        type=str,
        required=True,
        help="new file name after preprocessing",
    )
    parser.add_argument(
