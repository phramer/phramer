...
DATA_PATH=/data/pafakanov/gigaword/data-bin
CHECKPOINT_PATH=/data/pafakanov/checkpoints/gigaword_seq2seq

python ${FAIRSEQ_PATH}/train.py ${DATA_PATH} \
	   --no-enc-token-positional-embeddings --elmo-affine --share-decoder-input-output-embed \
	   --distributed-world-size 8 --max-update 30000 \
	   ...
	   --ddp-backend no_c10d --min-lr 1e-09 --clip-norm 0.0 --dropout 0.3 --weight-decay 0.0 \
           ...
           --elmo-dropout 0.2 --max-tokens 2000 --arch transformer_wmt_en_de --seed 1 --warmup-init-lr 1e-7 \
	   --encoder-embed-path elmo:${LM_CHECKPOINT_PATH}/checkpoint_best.pt --source-lang articles --target-lang summaries \
	   --save-interval-updates 500 --keep-interval-updates 10 --save-dir $CHECKPOINT_PATH --comet-logging \
